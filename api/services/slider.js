...
class SlidersService {
    ...
    async editSlider(
        ...
        await db('tbl_slider_directory').where({ id }).update({
            title,
            small_description: short_description,
            description,
            classified_address: link,
    ...
    async deleteSlider(id) {
        await db('tbl_slider_directory').where({ id }).del();
    }

    async getSliders(limit, page, language_id, is_active) {
        let posts;

        posts = await db('tbl_slider_directory')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getSlidersCount(language_id, is_active) {
        const count = await db('tbl_slider_directory')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
