...
class WatchesService {
    ...
    async editCategory(
        ...
        await db('tbl_directory_category').where({ id }).update({
            title,
            description,
            url_slug: slug,
            meta_title,
    ...
    async deleteCategory(id) {
        await db('tbl_directory_category').where({ id }).del();
    }

    async getCategories(limit, page, language_id, is_active) {
        let categories;

        categories = await db('tbl_directory_category')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getCategory(id) {
        const category = await db('tbl_directory_category')
            .where({ id })
            .first();
    ...
    async getCategoriesCount(language_id, is_active) {
        const categoriesCount = await db('tbl_directory_category')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async editPost(
        ...
        await db('tbl_classified_directory').where({ id }).update({
            title,
            description,
            classified_slug: slug,
            banner_image,
    ...
    async deletePost(id) {
        await db('tbl_classified_directory').where({ id }).del();
    ...
    async getPosts(limit, page, language_id, category_ids, is_active) {
        ...
        if (category_ids.length == 1) {
            posts = await db('tbl_classified_directory')
                .leftJoin(
                    'tbl_post_category_directory_map',
                    'tbl_classified_directory.id',
                    'tbl_post_category_directory_map.post_id'
                )
                .where(
                    'tbl_post_category_directory_map.category_id',
                    category_ids[0]
                )
                .andWhere({
                ...
                .modify(function (queryBuilder) {
                    if (is_active != 'all') {
                        queryBuilder.where(
                            'tbl_classified_directory.is_active',
                            is_active
                        );
                    }
                });
    ...
    async getPostWithSlug(slug) {
        const post = await db('tbl_classified_directory')
    ...
    async getPostImage(id) {
        const post = await db('tbl_classified_directory_image')
            .where({ id })
            .first();
    ...
    async getPostImages(post_id) {
        const post = await db('tbl_classified_directory_image').where({
            classified_id: post_id,
        });
    ...
    async editPostImageStatus(id, is_active) {
        await db('tbl_classified_directory_image').where({ id }).update({
            is_active,
        });
    }

    async deletePostImage(id) {
        await db('tbl_classified_directory_image').where({ id }).del();
    }
}
