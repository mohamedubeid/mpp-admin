...
class NewsService {
    ...
    async editCategory(
        ...
        await db('tbl_generic_category').where({ id }).update({
            title,
            url_slug: slug,
            description,
            meta_title,
    ...
    async deleteCategory(id) {
        await db('tbl_generic_category').where({ id }).del();
    }

    async getCategories(limit, page, language_id, is_active) {
        let categories;

        categories = await db('tbl_generic_category')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getCategoryWithSlug(slug) {
        const category = await db('tbl_generic_category')
            .where({ url_slug: slug })
            .first();
    ...
    async getCategoriesCount(language_id, is_active) {
        const count = await db('tbl_generic_category')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async editPost(
        ...
        await db('tbl_classified').where({ id }).update({
            title,
            classified_slug: slug,
            small_description: short_description,
            description,
        ...
        await db('tbl_post_category_map').where({ post_id: id }).del();
    ...
    async deletePost(id) {
        await db('tbl_classified').where({ id }).del();
    ...
    async getPostWithSlug(slug) {
        const post = await db('tbl_classified')
    ...
    async getPostCategories(post_id) {
        const categories = await db('tbl_post_category_map')
            ...
            .leftJoin(
                'tbl_generic_category',
                'tbl_post_category_map.category_id',
                'tbl_generic_category.id'
            )
            .where({
    ...
    async getPostImages(post_id) {
        const post = await db('tbl_classified_image').where({
            classified_id: post_id,
        });
    ...
    async editPostImageStatus(id, is_active) {
        await db('tbl_classified_image').where({ id }).update({
            is_active,
        });
    }

    async deletePostImage(id) {
        await db('tbl_classified_image').where({ id }).del();
    }

    async countSelectedPosts(language_id) {
        const selectedPostsCount = await db('tbl_classified').where({
            is_selected: '1',
            language_id,
        });
    ...
    async selectPost(id, is_selected) {
        const response = await db('tbl_classified').where({ id }).update({
            is_selected,
        });
        return response;
    }
    ...
    async getSelectedPost(language_id) {
        const posts = await db('tbl_classified')
            .where({
                is_selected: '1',
                language_id,
            })
