...
class PiecesService {
    ...
    async editPiece(id, title, banner_image, is_active, user_id) {
        await db('tbl_pieces_directory').where({ id }).update({
            title,
            banner_image,
            is_active,
            created_by: user_id,
    ...
    async deletePiece(id) {
        await db('tbl_pieces_directory').where({ id }).del();
    }

    async getPieces(limit, page, language_id, is_active) {
        let posts;

        posts = await db('tbl_pieces_directory')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getPieceWithSlug(slug) {
        const post = await db('tbl_pieces_directory')
    ...
    async getPiecesCount(language_id, is_active) {
        const count = await db('tbl_pieces_directory')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
