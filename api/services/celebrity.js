...
class CelebrityService {
    ...
    async editCategory(
        ...
        await db('tbl_celebrity_category').where({ id }).update({
            sub_title: '',
            category_image: '',
            parent_id: parent_page,
            created_date: new Date(),
    ...
    async deleteCategory(id) {
        await db('tbl_celebrity_category').where({ id }).del();
    }

    async getCategories(limit, page, language_id, is_active) {
        let categories;

        categories = await db('tbl_celebrity_category')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getCategory(id) {
        const category = await db('tbl_celebrity_category')
            .where({ id })
            .first();
    ...
    async getCategoriesCount(language_id, is_active) {
        const count = await db('tbl_celebrity_category')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async editPost(
        ...
        await db('tbl_celebrity_directory').where({ id }).update({
            title,
            description,
            classified_slug: slug,
            small_description: short_description,
    ...
    async deletePost(id) {
        await db('tbl_celebrity_directory').where({ id }).del();
    ...
    async getPosts(limit, page, language_id, category_ids, is_active) {
        ...
        if (category_ids.length == 1) {
            posts = await db('tbl_celebrity_directory')
                .leftJoin(
                    'tbl_post_celebrity_directory_map',
                    'tbl_celebrity_directory.id',
                    'tbl_post_celebrity_directory_map.post_id'
                )
                .where(
                    'tbl_post_celebrity_directory_map.category_id',
                    category_ids[0]
                )
                .andWhere({
                ...
                .modify(function (queryBuilder) {
                    if (is_active != 'all') {
                        queryBuilder.where(
                            'tbl_celebrity_directory.is_active',
                            is_active
                        );
                    }
                });
    ...
    async getPostWithSlug(slug) {
        const post = await db('tbl_celebrity_directory')
    ...
    async getPostCategories(post_id) {
        const categories = await db('tbl_post_celebrity_directory_map')
            ...
            .leftJoin(
                'tbl_celebrity_category',
                'tbl_post_celebrity_directory_map.category_id',
                'tbl_celebrity_category.id'
            )
            .where({
    ...
    async getPostImage(id) {
        const post = await db('tbl_celebrity_directory_image')
            .where({ id })
            .first();
    ...
    async getPostImages(post_id) {
        const post = await db('tbl_celebrity_directory_image').where({
            classified_id: post_id,
        });
    ...
    async editPostImageStatus(id, is_active) {
        await db('tbl_celebrity_directory_image').where({ id }).update({
            is_active,
        });
    }

    async deletePostImage(id) {
        await db('tbl_celebrity_directory_image').where({ id }).del();
    }

    async countSelectedPosts(language_id) {
        const selectedPostsCount = await db('tbl_celebrity_directory').where({
            is_selected: '1',
    ...
    async selectPost(id, is_selected) {
        const response = await db('tbl_celebrity_directory')
            .where({ id })
            .update({
                is_selected,
            });
    ...
    async getSelectedPost(language_id) {
        const posts = await db('tbl_celebrity_directory')
            .where({
                is_selected: '1',
                language_id,
            })
