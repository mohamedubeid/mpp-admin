...
class LifestylesService {
    ...
    async editLifestyle(
        ...
        await db('tbl_lifestyle_directory').where({ id }).update({
            section_id: 0,
            classifed_modal: '',
            amount: 0,
            target_date: new Date(),
    ...
    async deleteLifestyle(id) {
        await db('tbl_lifestyle_directory').where({ id }).del();
    }

    async getLifestyles(limit, page, language_id, is_active) {
        let posts;

        posts = await db('tbl_lifestyle_directory')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getLifestyleWithSlug(slug) {
        const post = await db('tbl_lifestyle_directory')
    ...
    async getLifestyleCount(language_id, is_active) {
        const count = await db('tbl_lifestyle_directory')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async getLifestyleImage(id) {
        const event = await db('tbl_lifestyle_directory_image')
            .where({ id })
            .first();
    ...
    async getLifestyleImages(lifestyle_id) {
        const event = await db('tbl_lifestyle_directory_image').where({
            classified_id: lifestyle_id,
        });
    ...
    async editLifestyleImageStatus(id, is_active) {
        await db('tbl_lifestyle_directory_image').where({ id }).update({
            is_active,
        });
    }
