...
class MagazinesService {
    ...
    async editMagazine(
        ...
        await db('tbl_magazine_directory').where({ id }).update({
            title,
            classified_slug: slug,
            description,
    ...
    async deleteMagazine(id) {
        await db('tbl_magazine_directory').where({ id }).del();
    }

    async getMagazines(limit, page, language_id, is_active) {
        let posts;
        posts = await db('tbl_magazine_directory')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getMagazineWithSlug(slug) {
        const post = await db('tbl_magazine_directory')
    ...
    async getMagazinesCount(language_id, is_active) {
        const count = await db('tbl_magazine_directory')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async getMagazineImages(event_id) {
        const event = await db('tbl_magazine_directory_image').where({
            classified_id: event_id,
        });
