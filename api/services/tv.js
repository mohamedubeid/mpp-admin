...
class TvService {
    ...
    async editCategory(
        ...
        await db('tbl_tv_category').where({ id }).update({
            title,
            description,
            url_slug: slug,
            meta_title,
    ...
    async deleteCategory(id) {
        await db('tbl_tv_category').where({ id }).del();
    }

    async getCategories(limit, page, language_id, is_active) {
        let categories;

        categories = await db('tbl_tv_category')
            .where({ language_id })
            .orderBy('id', 'desc')
            .offset((page - 1) * limit)
            .limit(limit)
    ...
    async getCategoryWithSlug(slug) {
        const category = await db('tbl_tv_category')
            .where({ url_slug: slug })
            .first();
    ...
    async getCategoriesCount(language_id, is_active) {
        const categoriesCount = await db('tbl_tv_category')
            .where({ language_id })
            .count('id as count')
            .modify(function (queryBuilder) {
                if (is_active != 'all') {
    ...
    async editVideo(
        ...
        await db('tbl_tv_directory').where({ id }).update({
            title,
            videoname: video,
            classified_slug: slug,
            banner_image,
        ...
        await db('tbl_post_tv_directory_map').where({ post_id: id }).del();
    ...
    async deleteVideo(id) {
        await db('tbl_tv_directory').where({ id }).del();
        ...
        if (category_ids.length == 1) {
            posts = await db('tbl_tv_directory')
                .leftJoin(
                    'tbl_post_tv_directory_map',
                    'tbl_tv_directory.id',
                    'tbl_post_tv_directory_map.post_id'
                )
    ...
    async getVideoWithSlug(slug) {
        const post = await db('tbl_tv_directory')
    ...
    async getVideoCategories(post_id) {
        const categories = await db('tbl_post_tv_directory_map')
            ...
            .leftJoin(
                'tbl_tv_category',
                'tbl_post_tv_directory_map.category_id',
                'tbl_tv_category.id'
            )
            .where({
    ...
    async countSelectedVideos(language_id) {
        const selectedVideosCount = await db('tbl_tv_directory').where({
            is_selected: '1',
            language_id,
        });
        return selectedVideosCount.length;
    }

    async selectVideo(id, is_selected) {
        const response = await db('tbl_tv_directory').where({ id }).update({
            is_selected,
        });
        return response;
    }
    async getSelectedVideos(language_id) {
        const posts = await db('tbl_tv_directory')
            .where({
                is_selected: '1',
