...
class NewsController {
    ...
    async createCategory(req, res) {
        ...
        let category;
        try {
            category = await NewsService.createCategory(
                title,
                description,
                slug,
                meta_title,
    ...
    async editCategory(req, res) {
        ...
        category = null;
        try {
            category = await NewsService.editCategory(
                id,
                title,
                description,
                slug,
        ...
        return {
            message: 'Successfully edited News Category!',
            category,
            statusCode: 200,
        };
    }
    ...
    async getAllCategories(req, res) {
        ...
        let categories = await NewsService.getCategories(
            limit,
            page,
            language_id,
            is_active
        ...
        let categoriesCount = await NewsService.getCategoriesCount(
            language_id,
            is_active
        );
    ...
    async createPost(req, res) {
        ...
        if (!title || !date || !meta_title || !meta_tags || !sort_order)
            return { error: 'Please fill out all fields', statusCode: 400 };
        ...
        let post;
        try {
            post = await NewsService.createPost(
                title,
                slug,
                short_description,
                description,
    ...
    async editPost(req, res) {
        ...
        post = null;
        try {
            post = await NewsService.editPost(
                id,
                formattedCategories,
                title,
                slug,
        ...
        return {
            message: 'Successfully edited News Post!',
            post,
            statusCode: 200,
        };
    }
    ...
    async getAllPosts(req, res) {
        ...
        let posts = await NewsService.getPosts(
            limit,
            page,
            language_id,
            categoryIDs,
        ...
        let postsCount = await NewsService.getPostsCount(
            language_id,
            categoryIDs,
            is_active
        );
    ...
    async createPostImage(req, res) {
        ...
        let image;
        try {
            image = await NewsService.createPostImage(
                post_id,
                image_name,
                caption,
                language_id,
        ...
        return {
            message: 'Successfully added News Image!',
            image,
            statusCode: 200,
        };
    }
    ...
    async editPostImageStatus(req, res) {
        ...
        if (!id)
            return { error: 'Please provide a news image ID', statusCode: 400 };
        if (typeof id !== 'number')
            return {
                error: 'Please provide a valid news image ID',
                statusCode: 400,
            };
        ...
        try {
            ...
            return {
                message: 'Error editing news image, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async selectPost(req, res) {
        ...
        const countSelectedPosts = await NewsService.countSelectedPosts(
            language_id
        );
        if (countSelectedPosts >= 4 && selected) {
            return {
                msg: 'You have already select the maximum number of News posts, which is 4 News posts',
                statusCode: 400,
            };
        }
        const selectValue = selected ? '1' : '0';
        const selectPost = await NewsService.selectPost(id, selectValue);
    ...
    async getAllSelectedPosts(req, res) {
        try {
            ...
            let posts;
            posts = await NewsService.getSelectedPost(language_id);
            if (posts.length === 0) {
                posts = await NewsService.getLastUpdatedPost(
                    language_id,
                    limit
                );
            }
