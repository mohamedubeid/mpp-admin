...
class NewsletterController {
    ...
    async createNewsletter(req, res) {
        ...
        let newsletter;
        try {
            newsletter = await NewsletterService.createNewsletter(
                full_name,
                company_name,
                email,
                city,
            ...
            return {
                message: 'Error creating newsletter, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async deleteNewsletter(req, res) {
        ...
        if (!id)
            return { error: 'Please provide a subscriber ID', statusCode: 400 };
        if (typeof id !== 'number')
            return {
                error: 'Please provide a valid subscriber ID',
                statusCode: 400,
            };
        ...
        if (!subscriber)
            return { error: 'Newsletter not found', statusCode: 404 };

        try {
            await NewsletterService.deleteNewsletter(id);
        } catch (error) {
            return {
                message: 'Error deleting subscriber, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async getAllNewsletter(req, res) {
        ...
        const subscribers = await NewsletterService.getNewsletters(
            limit,
            page,
            is_active
        );

        let subscribersCount = await NewsletterService.getNewslettersCount(
            is_active
        );
