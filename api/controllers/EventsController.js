...
class EventsController {
    ...
    async createEvent(req, res) {
        ...
        if (eventSameSlug)
            return {
                error: 'Event with that slug already exists, please choose another slug',
                statusCode: 400,
            };

        let event;
        try {
            event = await EventsService.createEvent(
                title,
                slug,
                telephone_number,
                event_date,
    ...
    async editEvent(req, res) {
        ...
        if (!id)
            return { error: 'Please provide an event ID', statusCode: 400 };
        if (typeof id !== 'number')
            return {
        ...
        let eventSameSlug = await EventsService.getEventWithSlug(slug);
        if (eventSameSlug && eventSameSlug.id != id)
            return {
                error: 'Event with that slug already exists, please choose another slug',
                statusCode: 400,
            };
        ...
        event = null;
        try {
            event = await EventsService.editEvent(
                id,
                title,
                slug,
                telephone_number,
            ...
            return {
                message: 'Error editing event, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async getAllEvent(req, res) {
        ...
        let eventsCount = await EventsService.getEventsCount(
            language_id,
            is_active
        );
    ...
    async getEventWithSlug(req, res) {
        ...
        if (!slug)
            return { error: 'Please provide a event slug', statusCode: 400 };
    ...
    async createEventImage(req, res) {
        ...
        if (!event_id || !caption || !is_active || !language_id)
            return { error: 'Please fill out all fields', statusCode: 400 };
        ...
        let image;
        try {
            image = await EventsService.createEventImage(
                event_id,
                image_name,
                caption,
                language_id,
        ...
        return {
            message: 'Successfully added Event Image!',
            image,
            statusCode: 200,
        };
    }
