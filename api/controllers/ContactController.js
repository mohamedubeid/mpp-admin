...
class ContactController {
    ...
    async createContact(req, res) {
        ...
        try {
            ...
            return {
                message: 'Error creating contact, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async deleteContact(req, res) {
        ...
        if (!id)
            return { error: 'Please provide a contact ID', statusCode: 400 };
        if (typeof id !== 'number')
            return {
                error: 'Please provide a valid contact ID',
                statusCode: 400,
            };
        ...
        if (!contact)
            return { error: 'Contact Request not found', statusCode: 404 };

        try {
            await ContactService.deleteContact(id);
        } catch (error) {
            return {
                message: 'Error deleting contact, please try again later',
                err: error,
                statusCode: 500,
            };
        }
    ...
    async getContact(req, res) {
        ...
        if (!id)
            return {
                error: 'Please provide a contact_request ID',
                statusCode: 400,
            };
        if (typeof id !== 'number')
            return {
                error: 'Please provide a valid contact_request ID',
                statusCode: 400,
            };
