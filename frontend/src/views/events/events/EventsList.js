...
const EventsList = () => {
  ...
  function updateData() {
    ...
    eventsService.getSelectedEvents(language_id).then((result) => {
      setSelectedEvents(result.data.selectedPosts)
    })
  }
  ...
  const handleSelectEvents = (e, post) => {
    const selected = e.target.checked
    if (selected && selectedEvents.length < 5) {
      setSelectedEvents([...selectedEvents, post])
      ...
      let temp2 = selectedEvents
      temp2 = temp2.filter((item) => item.id !== post.id)
      setSelectedEvents(temp2)
      let temp3 = eventsTable
      ...
      setEventsTable(temp3)
    } else {
      setErrMsg('You are selected the maximum number of Events posts')
      setDisplayNtf(true)
  ...
  const handleConfirmSelected = () => {
    eventsService.selectEvents(language_id, { selectedEvents }).then((result) => {
  ...
  const handleCancelSelected = () => {
    eventsService.ClearSelectedEvents(language_id).then((result) => {
      setSelectedEvents([])
      let temp3 = eventsTable
      temp3 = temp3.map((item) => {
        item.is_selected = '0'
                  ...
                  <CTableRow key={i}>
                    <CTableDataCell scope="row">
                      <CFormCheck
                        ...
                        onChange={(e) => handleSelectEvents(e, events)}
