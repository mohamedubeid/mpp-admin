...
import { cilPencil, cilTrash, cilPlus, cilImage } from '@coreui/icons'
...
const ImagesList = () => {
  ...
  function updateData() {
    jewelryService.getJewelryImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    jewelryService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/jewelry/post-images-add/"+params.id)}
            color={'primary'}
          >
